/*
 * Bootloader_hex.c
 * YONTEM: Once Yaz, Sonra Kontrol Et (Download First, Verify Later)
 */
...
#define VERSION_OFFSET          0x400 // Versiyonun durduğu yer
...
/* --- FLASH VE YARDIMCI FONKSIYONLAR --- */
...
static void Write_Config_And_Reset(uint32_t new_slot, uint32_t new_ver, uint32_t target_addr) {
    ...
    Ei.TypeErase=FLASH_TYPEERASE_PAGES; Ei.NbPages=1;
    
    if(CONFIG_PAGE_ADDR < FLASH_BANK2_START_ADDR) { 
        Ei.Banks=FLASH_BANK_1; Ei.Page=(CONFIG_PAGE_ADDR-FLASH_BASE)/FLASH_PAGE_SIZE; 
    } else { 
        Ei.Banks=FLASH_BANK_2; Ei.Page=(CONFIG_PAGE_ADDR-FLASH_BANK2_START_ADDR)/FLASH_PAGE_SIZE; 
    }
...
void Xmodem_Receive_Hex_File(void)
{
    ...
    printf("\r\n[HEX XMODEM] Hedef: 0x%08lX. Gonderin...\r\n", target_slot);
    ...
    while(!done) {
        ...
        /* --- BITTI (EOT) - SIMDI KONTROL ZAMANI --- */
        if(status == EOT) {
            HAL_UART_Transmit(&huart1, &ack, 1, 100);
            Flush_Smart_Buffer();
            
            /* 1. Versiyonu Oku (Flash'tan) */
            ...
            /* 2. Karsilastir ve Sor */
            printf("\r\n\r\n=== YUKLEME TAMAMLANDI ===\r\n");
            printf("Yeni Versiyon: %08lX (v%lu.%lu.%lu)\r\n", new_ver, (new_ver>>16)&0xFF, (new_ver>>8)&0xFF, new_ver&0xFF);
            printf("Eski Versiyon: %08lX\r\n", old_ver);
            
            if(new_ver == 0xFFFFFFFF) {
                printf(CLR_YELLOW "[UYARI] Versiyon bilgisi okunamadi! (App'te tanimladin mi?)\r\n" CLR_RESET);
            }

            if(new_ver > old_ver) printf(CLR_GREEN "Durum: YUKSELTME (Upgrade)\r\n" CLR_RESET);
            else if(new_ver < old_ver) printf(CLR_RED "Durum: SURUM DUSURME (Downgrade)!\r\n" CLR_RESET);
            else printf(CLR_YELLOW "Durum: AYNI SURUM (Re-install)\r\n" CLR_RESET);

            printf("Onayliyor musunuz? (e: Evet, h: Hayir) > ");
            ...
            while(1) {
                ...
                if(HAL_UART_Receive(&huart1, &rx, 1, HAL_MAX_DELAY) == HAL_OK) {
                    ...
                    if(rx=='h'||rx=='H') {
                        printf(CLR_RED "\r\n[IPTAL] Yeni surum aktif edilmedi.\r\n" CLR_RESET);
                        break; 
                    }
                }
            }
